<x-guest-layout>
    <x-auth-shell eyebrow="Secure verification" title="Verify your phone" description="Enter the one-time code to finish creating your account." panel-title="Welcome to the network." panel-description="A quick verification helps us keep your new account safe from the start.">
        ...
        <form method="POST" action="{{ route('verify') }}">
            @csrf
            <input type="hidden" name="temp_id" value="{{ $otpData->id }}">
        ...
        <form id="resend-new-otp" method="POST" action="{{ route('resend.otp') }}">@csrf<input type="hidden" name="temp_id" value="{{ $otpData->id }}"><input type="hidden" name="phone" value="{{ $otpData->phone }}"><input type="hidden" name="email" value="{{ $otpData->email }}"></form>
    </x-auth-shell>
</x-guest-layout>
