<x-guest-layout>
    <x-auth-shell eyebrow="Member account" title="Create your account" description="Join the Right community and start building your next chapter." panel-title="Start something meaningful." panel-description="A connected marketplace for people, products and possibilities.">
        ...
        <form method="POST" action="{{ route('user.otp') }}">
            ...
            <div class="mt-4"><x-input-label for="phone" :value="__('Phone number')" /><x-text-input id="phone" type="text" name="phone" :value="old('phone')" required autocomplete="tel" placeholder="01XXXXXXXXX" /><x-input-error :messages="$errors->get('phone')" class="auth-error" /></div>
            <div class="mt-4"><x-input-label for="email" :value="__('Email address')" /><x-text-input id="email" type="email" name="email" :value="old('email')" required autocomplete="username" placeholder="you@example.com" /><x-input-error :messages="$errors->get('email')" class="auth-error" /></div>
