<script>
    $(function() {
        ...
        $(document).on('click', '.address-card-wrapper', function() {
            ...
            updateSelectedCardStyle(); // Your custom function if needed
        });
        ...
        $('#place_order').on('click', function() {
            ...
            let selected_shipping_address_id = $('input[name="selected_shipping_address_id"]:checked').val();
            ...
            if (isShippingDifferent) {
                shipping = {
                    ...
                    post_office_id: $('select[name="shipping_address[delivery_post_office_id]"]').val(),
            ...
            const payment_method_id = paymentMethod.val();
            const payment_account_number = paymentMethod.closest('label').data('account');
            const transaction_id = $('#transaction_id').val();
            const sender_phone_number = $('#sender_phone_number').val();
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if(res.order_id){
                        window.location.href = "{{ route('order_success', ['order_id' => '__ORDER_ID__']) }}".replace('__ORDER_ID__', res.order_id);
                    }
                },
                error: function(xhr) {
